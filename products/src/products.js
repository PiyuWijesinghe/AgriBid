  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-800 to-green-400 text-white font-sans px-6 py-8">
      ...
      <nav className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
        <div className="text-3xl font-extrabold tracking-wide">AgriBid.lk</div>
      ...
      <section>
        <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-4">
          <h2 className="text-4xl font-bold">Available Products</h2>
          <input
            type="text"
            placeholder="Search products..."
            className="px-4 py-2 rounded-md text-black w-72 shadow-md focus:outline-none"
          />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product, index) => (
            <div
              key={index}
              className="bg-white/20 backdrop-blur-lg p-4 rounded-2xl shadow-lg text-center hover:scale-105 transition duration-200"
            >
              <img
                src={product.image}
                alt={product.name}
